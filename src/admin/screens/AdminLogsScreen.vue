        ...
        <div ref="filtersRef" class="relative z-40 w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6">
           ...
           overflow-hidden w-[200px]">
                            <div class="flex flex-col">
                                <!-- header row -->
                                <button @click="select('users', headerLabels.users)" class="w-full flex items-center justify-between px-4 py-2 text-[14px]
               text-[#0A6B7A] dark:text-[#8ED7E4] hover:bg-[#EAF8FB] dark:hover:bg-[#0D2C31]">
                                ...
                                <!-- options + dividers -->
                                <template v-for="(opt, i) in options.users" :key="`users-${opt}`">
                                    <button @click="select('users', opt)" class="w-full text-left px-4 py-2 text-[14px]
                 text-[#0C3E45] dark:text-[#A9F3FF]
                 hover:bg-[#EAF8FB] dark:hover:bg-[#0D2C31]">
                                        {{ opt }}
                                    </button>
                                    <div v-if="i !== options.users.length - 1"
                                        class="border-b border-[#00beec14] dark:border-[#00beec1f]">
                                    </div>
                                </template>
                            </div>
                        </div>
           ...
           overflow-hidden w-[200px]">
               ...
               text-[#0A6B7A] dark:text-[#8ED7E4] hover:bg-[#EAF8FB] dark:hover:bg-[#0D2C31]">
                 ...
                 hover:bg-[#EAF8FB] dark:hover:bg-[#0D2C31]">
                                    ...
                                    <div v-if="i !== options.action.length - 1"
                                        class="border-b border-[#00beec14] dark:border-[#00beec1f]">
                                    </div>
                                </template>
                            </div>
...
const headerLabels = { users: 'All Users', action: 'All Actions', date: 'Date Range' }
const options = { users: ['Jenny Wilson', 'Floyd Miles', 'Eleanor Pena', 'Jane Cooper', 'Brooklyn Simmons'], action: ['Login', 'Edit', 'Delete'], date: ['All Time', 'Last 7 days', 'Last 30 days', 'This Year'] }
const selected = reactive({ users: headerLabels.users, action: headerLabels.action, date: headerLabels.date })
const open = reactive({ users: false, action: false, date: false })
...
function clearAll() { selected.users = headerLabels.users; selected.action = headerLabels.action; selected.date = headerLabels.date; query.value = ''; chips.value = [] }
...
/* ---------- logs data ---------- */
const logs = ref([
    { time: '2025-04-23 12:44:31', name: 'Ronald Richards', role: 'Manager', avatar: avatar1, action: 'Login', ip: '192.168.1.112', client: 'Flex Corporation', desc: 'User logged into admin panel' },
    { time: '2025-04-23 12:44:31', name: 'Kristin Watson', role: 'User', avatar: avatar2, action: 'Edit', ip: '192.168.1.10.5', client: 'Mino Company', desc: 'Updated client profile information' },
    { time: '2025-04-23 12:44:31', name: 'Wade Warren', role: 'Manager', avatar: avatar3, action: 'Delete', ip: '192.168.3.245', client: 'Tom ventures', desc: 'Delete API Token' },
    { time: '2025-04-23 12:44:31', name: 'Kathryn Murphy', role: 'Admin', avatar: avatar4, action: 'Login', ip: '192.168.1.165', client: 'Aero airbus', desc: 'User logged into admin panel' },
    // extra rows for paging
    ...Array.from({ length: 16 }).map((_, i) => ({
        time: `2025-04-${String(10 + (i % 20)).padStart(2, '0')} 0${(i % 9) + 1}:3${i % 6}${i % 9}`,
        name: ['Ronald Richards', 'Kristin Watson', 'Wade Warren', 'Kathryn Murphy'][i % 4],
        role: ['Manager', 'User', 'Admin'][i % 3],
        avatar: [avatar1, avatar2, avatar3, avatar4][i % 4],
        action: ['Login', 'Edit', 'Delete'][i % 3],
        client: ['Flex Corporation', 'Mino Company', 'Tom ventures', 'Aero airbus'][i % 4],
...
function roleBadge(role) {
    const base = 'text-xs font-medium rounded-full px-2 py-0.5';
    if (role === 'Manager') return `${base} bg-[#C300FF] text-white dark:bg-[#C300FF26] dark:text-[#C300FF]`;
    if (role === 'User') return `${base} bg-[#00D9FF] text-white dark:bg-[#00D9FF26] dark:text-[#00D9FF]`;
    if (role === 'Admin') return `${base} bg-[#FFC800] text-white dark:bg-[#FFC80026] dark:text-[#FFC800]`;
