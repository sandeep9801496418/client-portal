<template>
    <main class="bg-light-bg dark:bg-dark-bg min-h-screen text-black dark:text-white overflow-y-hidden">
        ...
        <header class="px-4 sm:px-6 py-4 sm:py-5 border-b border-[rgba(0,190,236,0.30)]">
            <div class="flex items-center justify-between">
                <div class="flex flex-col sm:flex-row sm:items-end gap-1 sm:gap-4">
                    <h1 class="font-inter text-[20px] sm:text-[24px] md:text-[30px] font-medium leading-none">Tokens Management</h1>
        ...
        <!-- Filters / actions row -->
        <div ref="filtersRef" class="relative z-40 w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4">
            <div class="mt-2 flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center gap-4 justify-between">
                ...
                <div class="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
                 ...
                 hover:bg-[#EAF8FB] dark:hover:bg-[#0D2C31]">
                    ...
                    <div class="relative">
                        <button @click="toggle('clients')" class="h-[37px] w-full sm:w-auto md:w-[165px] px-3 sm:px-5 rounded-[6px] flex items-center justify-between gap-2
                   bg-light-bg/80 dark:bg-[#0A2327]
                   text-sm sm:text-[14px] text-black/80 dark:text-primary border dark:border-none">
                            {{ selected.clients }}
                            <img src="../../images/client-portal/arrow-close.svg"
                                :class="['transition-transform w-3 h-3 sm:w-4 sm:h-4', open.clients ? 'rotate-180' : '']" alt="" />
                        </button>
           ...
           bg-light-bg/80 dark:bg-[#0A2327]
           text-xs sm:text-sm md:text-[14px] text-black/80 dark:text-primary border dark:border-none">
                        ...
                        <!-- Calendar panel -->
                        <div v-if="open.date" class="absolute z-50 mt-2 left-0 sm:right-auto w-[280px] sm:w-[320px] md:w-[360px] rounded-[12px] p-3 sm:p-4 overflow-hidden
        ...
        <section class="mt-5 rounded-2xl border z-10
                  ...
                  dark:border-[#00BEEC66] dark:bg-[#FFFFFF0D] dark:text-white backdrop-blur m-3 sm:m-6 overflow-x-auto">
            <!-- Head -->
            <div class="px-3 sm:px-5 md:px-6 py-2 m-3 sm:m-5 border border-[#00beec3d] dark:border-[#00BEEC66]
                ...
                <div class="grid grid-cols-[1.4fr,1fr,1fr,1.8fr,1fr,1fr] gap-2 sm:gap-4
                  text-[13px] sm:text-[15px] md:text-[18px] font-medium text-[#007D95] dark:text-primary">
                    <span class="min-w-[120px]">Token Name/ID</span>
                    <span class="min-w-[80px]">Client</span>
                    <span class="min-w-[80px]">Status</span>
                    <span class="min-w-[120px]">Usage</span>
                    <span class="min-w-[100px]">Expiration</span>
                    <span class="text-right pr-2 min-w-[100px]">Actions</span>
                </div>
            </div>

            <!-- Rows -->
            <div class="px-1 sm:px-2 md:px-3 mx-3 sm:mx-5 min-w-[900px]">
                <div v-for="t in pagedTokens" :key="t.id"
                    class="grid grid-cols-[1.4fr,1fr,1fr,1.8fr,1fr,1fr] gap-2 items-center py-3 min-w-[900px]">
                    <!-- name / id -->
                    ...
                    <div class="min-w-[80px]">
                        <div class="font-medium text-sm sm:text-base"> Hellop </div>
                    </div>
                    ...
                    <!-- usage -->
                    <div class="flex items-center gap-2 sm:gap-4 min-w-[120px]">
                        <div class="min-w-[50px] sm:min-w-[60px] text-right text-sm sm:text-base">{{ formatNumber(t.usage) }}</div>
                        <div
                            class="relative w-[80px] sm:w-[110px] h-[12px] sm:h-[16px] rounded-full bg-black/10 dark:bg-light-bg/10 overflow-hidden">
